...
def test(exp_settings):
    ...
    qdfile = open('ULTRE_train_sample_1_click/test-on-train/test.init_list', 'r')
    query = []
    for line in qdfile:
        if (line != ''):
            q = int(line.strip().split(':')[0])
    ...
    labelfile = open('ULTRE_train_sample_1_click/test-on-train/test.labels', 'r')
    label_dict = {}
