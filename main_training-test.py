...
def test(exp_settings):
    ...
    model = DLA_DCM(test_set, exp_settings)
    # checkpoint_path = os.path.join(args.model_dir+'_256_0.1', "%s.ckpt8150" % exp_settings['learning_algorithm'])
    ...
    qdfile = open('ULTRE_train_pl_0_click/training_test/training_test.init_list', 'r')
    query = []
    for line in qdfile:
        if (line != ''):
            q = int(line.strip().split(':')[0])
    ...
    labelfile = open('ULTRE_train_pl_0_click/training_test/training_test.labels', 'r')
    label_dict = {}
