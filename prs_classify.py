...
for w in ws:
    result_path = 'train_pl_{}_prs_result.txt'.format(w)
    ...
    for k in model_ssdic.keys():
        ...
        model_reset = k
        algorithm = 'PRSrank_modify'
        rs = os.popen("python main_2.py --train_data_prefix train --setting_file ./ULTRE_train_pl_-2_click/prs_exp_settings.json \
        --batch_size 300 --test_only True --data_dir ./ULTRE_train_pl_-2_click/ \
        --model_dir ./ULTRE_train_pl_{}_click/model/{}_256_{}/ultra.learning_algorithm.{}.ckpt{}".format(w, model_reset, lr, algorithm, max_valid_step))
        print(rs.read())
        rs.close()
        ...
        rs=os.popen("python main_training-test.py --test_data_prefix training_test --setting_file ./ULTRE_train_pl_-2_click/prs_exp_settings.json \
                   --batch_size 700 --test_only True --data_dir ./ULTRE_train_pl_0_click/ \
                   --model_dir ./ULTRE_train_pl_{}_click/model/{}_256_{}/ultra.learning_algorithm.{}.ckpt{}".format(w, model_reset, lr, algorithm, max_training_valid_step))
        print(rs.read())
        rs.close()
