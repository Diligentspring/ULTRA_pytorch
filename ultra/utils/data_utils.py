...
class Raw_data:
    ...
    def load_data_in_IPS_format(self, data_path=None, file_prefix=None, rank_cut=None):
        ...
        for line in feature_fin:
            ...
            self.dids.append(int(arr[0]))
            # self.features.append(np.zeros(self.feature_size))
            self.features[int(arr[0])] = [0.0 for _ in range(self.feature_size)]
            for x in arr[1:]:
                arr2 = x.split(':')
                feautre_idx = int(arr2[0])
                if feautre_idx < self.feature_size:
                    self.features[int(arr[0])][int(feautre_idx)] = float(arr2[1])
            for rf_idx in self.removed_feature_ids:
                del self.features[int(arr[0])][rf_idx - 1]
        ...
        for line in init_list_fin:
            ...
            if rank_cut:
                self.initial_list.append([int(x) for x in arr[1].strip().split(' ')[:rank_cut]])
            else:
                self.initial_list.append([int(x) for x in arr[1].strip().split(' ')])
