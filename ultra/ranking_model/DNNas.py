...
class DNNas(nn.Module):
    ...
    def __init__(self, hparams_str, feature_size):
        ...
        self.act_func = None
        self.output_sizes = self.hparams.hidden_layer_sizes + [2]
        ...
        self.sigmoid = BaseRankingModel.ACT_FUNC_DIC['sigmoid']
        ...
        for j in range(len(self.output_sizes)):
            ...
            if j != len(self.output_sizes) - 1:
                ...
                self.sequential.add_module('act{}'.format(j), self.sigmoid)
    ...
    def build(self, input_list, noisy_params=None,
        ...
        if (noisy_params == None):
            output_data = self.sequential(input_data)#2560*2
        else:
