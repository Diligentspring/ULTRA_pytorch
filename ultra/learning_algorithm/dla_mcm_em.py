...
class DLA_MCM_EM(BaseAlgorithm):
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def train(self, input_feed):
            ...
            for j in range(self.rank_list_size):
                if int(labels[i][j].data) == 1:
                    if last_click == 0:
                        ...
                        self.propensity[i][j] = self.propensity_parameter[j*10+last_click] * (1 - train_sat[i][last_click-1])
                    last_click = j+1
        ...
        for i in range(len(labels)):
            ...
            for j in range(self.rank_list_size):
                if int(labels[i][j].data) == 1:
                    #print(self.propensity[i][j])
                    exam_queryloss.append(torch.log(self.propensity[i][j] * train_rel[i][j]))
                    last_click = j + 1
                else:
                    if last_click == 0:
                        exam_queryloss.append(torch.log(1 - self.propensity_parameter[j*10+last_click] * train_rel[i][j]))
                    else:
                        exam_queryloss.append(torch.log(1 - self.propensity_parameter[j*10+last_click] * (1 - train_sat[i][last_click-1]) * train_rel[i][j]))
            #print(exam_queryloss)
            exam_batchloss.append(sum(exam_queryloss))
        self.exam_loss = -sum(exam_batchloss) / len(labels)
