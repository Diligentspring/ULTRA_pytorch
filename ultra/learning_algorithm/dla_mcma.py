...
from ultra.ranking_model.DNNas import DNNas
import ultra.utils
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def __init__(self, data_set, exp_settings):
        ...
        if 'selection_bias_cutoff' in exp_settings.keys():
            ...
            self.propensity_para = [torch.tensor([0.0]) for i in range(self.rank_list_size**2)]
        #self.model = self.create_model(self.feature_size)
        self.model = DNNas(self.exp_settings['ranking_model_hparams'], self.feature_size)
    ...
    def separate_gradient_update(self):
        ...
        for i in range(len(self.propensity_para)):
            if self.propensity_para[i].grad != None:
                if self.propensity_para[i].grad <= 5.0 and self.propensity_para[i].grad >= -5.0:
                    self.propensity_para[i].data = self.propensity_para[i].data - self.propensity_learning_rate * self.propensity_para[i].grad
                elif self.propensity_para[i].grad > 5.0:
                    print('grad', self.propensity_para[i].grad)
                    self.propensity_para[i].data = self.propensity_para[i].data - self.propensity_learning_rate * 5.0
                else:
                    print('grad', self.propensity_para[i].grad)
                    self.propensity_para[i].data = self.propensity_para[i].data - self.propensity_learning_rate * -5.0
    ...
    def train(self, input_feed):
        ...
        rank_scores = self.get_ranking_scores(model = self.model, input_id_list= self.docid_inputs[:self.rank_list_size])
        ...
        for i in range(len(rank_scores)):
            #print(rank_scores[i].shape)
            x = torch.unbind(rank_scores[i], dim=1)
            #print(x)
            #print(x[0].shape)
            list1.append(x[0])
            list2.append(x[1])
        #print(list1)
            #print(x[0])
            #print(x[0].shape)
        train_output = torch.stack(list1, 1) * torch.stack(list2, 1)
        with torch.no_grad():
            train_rel = torch.stack(list1, 1)
            train_sat = torch.stack(list2, 1)
            self.train_rel = train_rel
            self.train_sat = train_sat
        #print(train_rel)
        #print(train_sat)
        #print(train_output)
        ...
        #print(labels)
        self.propensity = torch.zeros_like(self.labels)
        # propensity_ips = np.zeros_like(labels)
        # costs = np.zeros_like(labels)
        # print(propensity_ips)
        for i in range(len(labels)):
            gammaList = []
            last_click = 0
            for j in range(self.rank_list_size):
                gammaList.append(self.propensity_parameter[j*10+last_click])
                if int(labels[i][j].data) == 1:
                    last_click = j+1
            alphaList = train_rel[i]
            s_cList = train_sat[i]
            #print(alphaList)

            f = [torch.tensor([1.0, 0]).to(device=self.cuda)]
            # b = [torch.tensor([1.0, 1.0]).to(device=self.cuda) for _ in range(self.rank_list_size)]
            z = [torch.tensor([1.0]).to(device=self.cuda)]
            ...
            # forward
            for j in range(self.rank_list_size):
                if int(labels[i][j].data) == 0:
                    t0 = torch.tensor([1.0]).to(device=self.cuda) - gammaList[j] + gammaList[j] * (torch.tensor([1.0]).to(device=self.cuda) - alphaList[j])
                    t1 = torch.tensor([0.0]).to(device=self.cuda)
                    p.append([t0, t1])
                    ...
                    p.append([
                        gammaList[j] * alphaList[j] * (torch.tensor([1.0]).to(device=self.cuda) - s_cList[j]),
                        gammaList[j] * alphaList[j] * s_cList[j]
                    ])
                    f.append([
                        f[j][0] * p[j][0],
                        f[j][0] * p[j][1]
            ...
            for j in range(self.rank_list_size):
                if int(labels[i][j].data) == 1:
                    if last_click == 0:
                        self.propensity[i][j] = self.propensity_parameter[j*10+last_click]
                    else:
                        self.propensity[i][j] = self.propensity_parameter[j*10+last_click] * f[j][0]
                    last_click = j+1
                ...
                for j in range(self.rank_list_size):
                    if (self.propensity_weights[i][j].data!=0.0):
                        if(self.propensity_weights[i][j].data > 0.01):
                            self.propensity_weights[i][j] = torch.tensor([1.0]).to(device=self.cuda) / self.propensity_weights[i][j]
                        else:
        ...
        #print(self.exam_loss)
        #print(self.rank_loss)
        self.loss = self.exam_loss + self.hparams.ranker_loss_weight * self.rank_loss
    ...
    def validation(self, input_feed, is_online_simulation=False):
        ...
        with torch.no_grad():
            rank_scores = self.get_ranking_scores(model=self.model,
                                                  input_id_list=self.docid_inputs[:self.max_candidate_num])
            list1 = []
            list2 = []
            for i in range(len(rank_scores)):
                x = torch.unbind(rank_scores[i], dim=1)
                list1.append(x[0])
                list2.append(x[1])
            self.output = torch.stack(list1, 1) * torch.stack(list2, 1)
