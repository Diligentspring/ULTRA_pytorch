...
from ultra.ranking_model.DNNa import DNNa
import ultra.utils
...
class DLA_DCMa(BaseAlgorithm):
    ...
    * Qingyao Ai, Keping Bi, Cheng Luo, Jiafeng Guo, W. Bruce Croft. 2018. Unbiased Learning to Rank with Unbiased Propensity Estimation. In Proceedings of SIGIR '18
    ...
    def __init__(self, data_set, exp_settings):
        ...
        self.model = DNNa(self.exp_settings['ranking_model_hparams'], self.feature_size)
        ...
        for i in range(len(labels)):
            ...
            for j in range(self.rank_list_size):
                if int(labels[i][j].data) == 1:
                    if last_click == -1:
                        self.propensity[i][j] = 1.0
                    else:
                        p = [self.propensity_parameter[last_click]]
                        for k in range(last_click + 1, j):
                            p.append( (1 - train_rel[i][k]) * p[-1] / (1 - train_rel[i][k] * p[-1]) )
                        self.propensity[i][j] = p[-1]
                    last_click = j
