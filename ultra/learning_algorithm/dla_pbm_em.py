...
class DLA_PBM_EM(BaseAlgorithm):
        ...
        for i in range(len(labels)):
            ...
            for j in range(self.rank_list_size):
                if int(labels[i][j].data) == 1:
                    exam_queryloss.append(torch.log(self.propensity_parameter[j] * train_rel[i][j]))
                else:
                    exam_queryloss.append(torch.log(1 - self.propensity_parameter[j] * train_rel[i][j]))
